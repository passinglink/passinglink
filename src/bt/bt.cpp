...
static const struct bt_data pl_bt_adv[] = {
    BT_DATA_BYTES(BT_DATA_FLAGS, (BT_LE_AD_GENERAL | BT_LE_AD_NO_BREDR)),
    BT_DATA_BYTES(BT_DATA_UUID16_ALL, 0x0d, 0x18, 0x0f, 0x18, 0x05, 0x18),
    BT_DATA_BYTES(BT_DATA_UUID128_ALL, 0x00, 0x00, 0x00, 0x00, PL_BT_UUID_PREFIX),
};

static struct bt_le_adv_param pl_bt_adv_params =
...
void bluetooth_init() {
  ...
  if (err != 0) {
    LOG_ERR("initialization failed: error = %02x", err);
    return;
  }

  err = bt_set_name("Passing Link");
  if (err != 0) {
    LOG_ERR("bt_set_name failed: error = %02x", err);
    return;
  }
  ...
  if (err != 0) {
    LOG_ERR("bt_passkey_set failed: error = %02x", err);
    return;
  } else {
  ...
  err = bt_le_adv_start(&pl_bt_adv_params, pl_bt_adv, ARRAY_SIZE(pl_bt_adv), nullptr, 0);
  if (err) {
    LOG_ERR("advertising failed to start: error = 0x%02x", err);
    return;
  }
}
